...
void FilterResult::print() {
    std::cout <<  "reads passed filter: " << mFilterReadStats[PASS_FILTER] << std::endl;
    std::cout <<  "reads failed due to low quality: " << mFilterReadStats[FAIL_QUALITY] << std::endl;
    std::cout <<  "reads failed due to too many N: " << mFilterReadStats[FAIL_N_BASE] << std::endl;
    if(mOptions->lengthFilter.enabled) {
        std::cout <<  "reads failed due to too short: " << mFilterReadStats[FAIL_LENGTH] << std::endl;
        if(mOptions->lengthFilter.maxLength > 0)
            std::cout <<  "reads failed due to too long: " << mFilterReadStats[FAIL_TOO_LONG] << std::endl;
    }
    if(mOptions->complexityFilter.enabled) {
        std::cout <<  "reads failed due to low complexity: " << mFilterReadStats[FAIL_COMPLEXITY] << std::endl;
    }
    if(mOptions->adapter.enabled) {
        std::cout <<  "reads with adapter trimmed: " << mTrimmedAdapterRead << std::endl;
        std::cout <<  "bases trimmed due to adapters: " << mTrimmedAdapterBases << std::endl;
    }
    if(mOptions->polyXTrim.enabled) {
        std::cout <<  "reads with polyX in 3' end: " << getTotalPolyXTrimmedReads() << std::endl;
        std::cout <<  "bases trimmed in polyX tail: " << getTotalPolyXTrimmedBases() << std::endl;
    }
    if(mOptions->correction.enabled) {
        std::cout <<  "reads corrected by overlap analysis: " << mCorrectedReads << std::endl;
        std::cout <<  "bases corrected by overlap analysis: " << getTotalCorrectedBases() << std::endl;
    }
}
...
void FilterResult::reportJson(ofstream& ofs, string padding) {
    ...
    ofs << padding << "\t" << "\"passed_filter_reads\": " << mFilterReadStats[PASS_FILTER] << "," << std::endl;
    if(mOptions->correction.enabled) {
        ofs << padding << "\t" << "\"corrected_reads\": " << mCorrectedReads << ","  << std::endl;
        ofs << padding << "\t" << "\"corrected_bases\": " << getTotalCorrectedBases() << ","  << std::endl;
    }
    ofs << padding << "\t" << "\"low_quality_reads\": " << mFilterReadStats[FAIL_QUALITY] << "," << std::endl;
    ofs << padding << "\t" << "\"too_many_N_reads\": " << mFilterReadStats[FAIL_N_BASE] << "," << std::endl;
    if(mOptions->complexityFilter.enabled)
        ofs << padding << "\t" << "\"low_complexity_reads\": " << mFilterReadStats[FAIL_COMPLEXITY] << "," << std::endl;
    ofs << padding << "\t" << "\"too_short_reads\": " << mFilterReadStats[FAIL_LENGTH] << "," << std::endl;
    ofs << padding << "\t" << "\"too_long_reads\": " << mFilterReadStats[FAIL_TOO_LONG] << std::endl;
...
void FilterResult::reportAdapterJson(ofstream& ofs, string padding) {
    ...
    ofs << padding << "\t" << "\"adapter_trimmed_reads\": " << mTrimmedAdapterRead << "," << std::endl;
    ofs << padding << "\t" << "\"adapter_trimmed_bases\": " << mTrimmedAdapterBases << "," << std::endl;
    ofs << padding << "\t" << "\"read1_adapter_sequence\": \"" << mOptions->getAdapter1() << "\"," << std::endl;
    if(mOptions->isPaired()) {
        ofs << padding << "\t" << "\"read2_adapter_sequence\": \"" << mOptions->getAdapter2() << "\"," << std::endl;
    }
...
void writeBaseCountsJson(ofstream& ofs, string pad, string key, long total, long (&counts)[4]) {
  ofs << pad << "\t\"total_" << key << "\": " << total << "," << std::endl;
  ofs << pad << "\t\"" << key << "\":{";
  for (int b=0; b<4; b++) {
    if(b > 0)
      ofs << ", ";
