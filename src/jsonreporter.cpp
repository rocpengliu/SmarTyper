...
//
//        ofs << "\t" << "\"kmer_detection_result\": {" << std::endl;
//        ofs << "\t\t" << "\"result\": \"" << detectionResult << "\"," << std::endl;
//        ofs << "\t\t" << "\"mean_coverage\": " << kmer->getMeanHit() << "," << std::endl;
//        ofs << "\t\t" << "\"positive_thread\": " << mOptions->positiveThreshold << "," << std::endl;
//
//        // unique kmer hits
//        ofs << "\t\t" << "\"kmer_hits\": {" << std::endl;
//            kmer->reportJSON(ofs);
//        ofs << "\t\t" << "}" << std::endl;
//
//
...
//    }
    ...
    ofs << "\t\t\t" << "\"total_reads\":" << pre_total_reads << "," << std::endl; 
    ofs << "\t\t\t" << "\"total_bases\":" << pre_total_bases << "," << std::endl; 
    ofs << "\t\t\t" << "\"q20_bases\":" << pre_q20_bases << "," << std::endl; 
    ofs << "\t\t\t" << "\"q30_bases\":" << pre_q30_bases << "," << std::endl; 
    ofs << "\t\t\t" << "\"q20_rate\":" << (pre_total_bases == 0?0.0:(double)pre_q20_bases / (double)pre_total_bases) << "," << std::endl; 
    ofs << "\t\t\t" << "\"q30_rate\":" << (pre_total_bases == 0?0.0:(double)pre_q30_bases / (double)pre_total_bases) << "," << std::endl; 
    ofs << "\t\t\t" << "\"read1_mean_length\":" << preStats1->getMeanLength() << "," << std::endl;
    if(mOptions->isPaired())
        ofs << "\t\t\t" << "\"read2_mean_length\":" << preStats2->getMeanLength() << "," << std::endl;
    ofs << "\t\t\t" << "\"gc_content\":" << (pre_total_bases == 0?0.0:(double)pre_total_gc / (double)pre_total_bases)  << std::endl; 
    ofs << "\t\t" << "}," << std::endl;

    ofs << "\t\t" << "\"after_filtering\": {" << std::endl;
    ofs << "\t\t\t" << "\"total_reads\":" << post_total_reads << "," << std::endl; 
    ofs << "\t\t\t" << "\"total_bases\":" << post_total_bases << "," << std::endl; 
    ofs << "\t\t\t" << "\"q20_bases\":" << post_q20_bases << "," << std::endl; 
    ofs << "\t\t\t" << "\"q30_bases\":" << post_q30_bases << "," << std::endl; 
    ofs << "\t\t\t" << "\"q20_rate\":" << (post_total_bases == 0?0.0:(double)post_q20_bases / (double)post_total_bases) << "," << std::endl; 
    ofs << "\t\t\t" << "\"q30_rate\":" << (post_total_bases == 0?0.0:(double)post_q30_bases / (double)post_total_bases) << "," << std::endl; 
    ofs << "\t\t\t" << "\"read1_mean_length\":" << postStats1->getMeanLength() << "," << std::endl;
    if(mOptions->isPaired())
        ofs << "\t\t\t" << "\"read2_mean_length\":" << postStats2->getMeanLength() << "," << std::endl;
    ofs << "\t\t\t" << "\"gc_content\":" << (post_total_bases == 0?0.0:(double)post_total_gc / (double)post_total_bases)  << std::endl; 
    ofs << "\t\t" << "}";
