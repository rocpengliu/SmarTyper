...
from ..utils.utils_func import get_trimmed_codingpos, split_codingpos, check_overlapping_gene, get_splicer_exon_list_map, get_new_codingpos
from ..utils.utils_common import print_time
from .ref_microtype_class import RefMicroType
import pdb
...
class MetaDataClass:
    ...
    def read_locifile2(self, parameter_class, post_microhap_class, mh=False):
        ...
        if os.path.isfile(fpath):
            ...
            if parameter_class.get_analtype() == "snp":
                for _, row in locitable.iterrows():
                    ...
                    mar_ref.set_cur_dna_ref(row['ref'].strip())
                    if mar_ref.get_trimr() >= len(mar_ref.get_ori_dna_ref()):
                    ...
                    if mar_ref.get_trimr() != 0:
                        mar_ref.set_cur_dna_ref(mar_ref.get_cur_dna_ref()[:-mar_ref.get_trimr()])
                    if mar_ref.get_triml() >= len(mar_ref.get_cur_dna_ref()):
                    ...
                    if mar_ref.get_triml() != 0:
                        mar_ref.set_cur_dna_ref(mar_ref.get_cur_dna_ref()[mar_ref.get_triml():])
                    if len(mar_ref.get_cur_dna_ref()) == 0:
                    ...
                    if pd.notna(row['snppos']) and str(row['snppos']).replace(' ', '') not in ["0", "0.0", "na", "nan", ""]:
                        ...
                        if len(snp_pos) != 0:
                            ...
                            trimmed_snp_pos = []
                            if mar_ref.get_trimr() != 0:
                                trimmed_snp_pos = [pos for pos in snp_pos if pos < (len(mar_ref.get_ori_dna_ref()) - mar_ref.get_trimr())]
                            if mar_ref.get_triml() != 0:
                                trimmed_snp_pos = [(pos - mar_ref.get_triml()) for pos in trimmed_snp_pos if (pos >= mar_ref.get_triml())]
                            if len(trimmed_snp_pos) != 0:
                                mar_ref.set_cur_snp_pos(trimmed_snp_pos)
                            else:
                                mar_ref.set_cur_snp_pos(snp_pos)
                    ...
                    if mh:
                        ...
                        if ori_exon_pos != '0' and ori_exon_pos is not None and ori_exon_pos != '' and ori_exon_pos.lower() != 'na' and ori_exon_pos.lower() != 'nan':
                            ...
                            if ori_exon_pos is not None and len(ori_exon_pos) != 0:
                                ...
                                mar_ref.set_ori_exon_pos(ori_exon_pos)
                                mar_ref.set_cur_exon_pos(ori_exon_pos)
                                if mar_ref.get_triml() != 0 or mar_ref.get_trimr() != 0:
                                    cur_exon_pos = get_trimmed_codingpos(ori_exon_pos, mar_ref.get_triml(), mar_ref.get_trimr(), len(mar_ref.get_ori_dna_ref()))
                                    mar_ref.set_cur_exon_pos(cur_exon_pos)
                                if mar_ref.get_cur_exon_pos() is not None and len(mar_ref.get_cur_exon_pos()) != 0: 
                                    mar_ref.set_has_exon(True)
                                mar_ref.populate_ref_mt()
