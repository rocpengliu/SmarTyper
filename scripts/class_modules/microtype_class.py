...
class MicroType:
    def __init__(self):
        ...
        self._var_pos_list = [] #snp or sap position
        self._var_nm_str = "" #snp or sap string
    ...
    def get_var_pos_list(self):
        return self._var_pos_list
    def set_var_pos_list(self, var_pos_list):
        self._var_pos_list = var_pos_list
    def get_var_nm_str(self):
        return self._var_nm_str
    def set_var_nm_str(self, var_nm_str):
        self._var_nm_str = var_nm_str
    def get_is_indel(self):
        return self._is_indel
    def set_is_indel(self, is_indel):
        self._is_indel = is_indel
        
class ComboMicroType:
    def __init__(self):
        self._mar = "" #marker
        self._has_mp = False #if it has mp, then it must have mh, but if it has mh, it may not have mp
        self._microhap = MicroType() # MicroType if it has mh, but may not have mp
        self._micropep = MicroType() # MicroTypeif it has mp, then it must have mh, but if it has mh, it may not have mp
    def get_mar(self):
        return self._mar
    def set_mar(self, mar):
        self._mar = mar
    def get_has_mp(self):
        return self._has_mp
    def set_has_mp(self, has_mp):
        self._has_mp = has_mp
    def get_microhap(self):
        return self._microhap
    def set_microhap(self, microhap):
        if not isinstance(microhap, MicroType):
            raise ValueError(f"microhap must be an instance of MicroType, but got {type(microhap)}")
        self._microhap = microhap
    def get_micropep(self):
        return self._micropep
    def set_micropep(self, micropep):
        if not isinstance(micropep, MicroType):
            raise ValueError(f"micropep must be an instance of MicroType, but got {type(micropep)}")
        self._micropep = micropep
