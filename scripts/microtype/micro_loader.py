...
def create_header(frame):
    ...
    label = ctk.CTkLabel(header_frame, text="Microhap loading", font=("Helvetica", 30, "bold"),
...
def create_body(parent, frame):
    ...
    ctk.CTkLabel(body_frame.top_panel, text="Loci file:", font=body_frame.bfont, text_color="white").grid(row=row, column=0, padx=body_frame.padx, pady=(1,1), sticky="e")
    ...
    ctk.CTkButton(body_frame.top_panel, text="Browse",font=body_frame.brfont, height=20, width=50, 
                  command=lambda: infile_browser(body_frame.top_panel.loci_entry, "index")).grid(row=row, column=1, pady=(1,1), sticky="w")
    ...
    ctk.CTkCheckBox(body_frame.top_panel, text="reverse complement", variable = loci_rev_var, font =body_frame.bfont, text_color="white").grid(row=row, column=3, padx=(0, 0),sticky="w")
    ...
    ctk.CTkLabel(body_frame.top_panel, text="Microhap input file:", font=body_frame.bfont, text_color="white").grid(row=row, column=0, padx=body_frame.padx, pady=(1,1), sticky="e")
    ...
    ctk.CTkLabel(body_frame.top_panel, text="Pre microhap file (optional):", font=body_frame.bfont, text_color="white").grid(row=row, column=0, padx=body_frame.padx, pady=(1,1), sticky="e")
...
def on_click_next_button(parent, footer_frame):
    if footer_frame.next_button.cget('state') == 'normal':
        ...
        parent.master.show_page("micro_res")