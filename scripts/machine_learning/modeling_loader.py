...
import pdb
from ..utils.modern_messagebox import showsuccess
...
def create_body(parent, frame):
    ...
    body_frame.top_panel.confirm_btn = ctk.CTkButton(body_frame.top_panel, text="Confirm",font=confirm_button_font, height=parent_button_size['height'], width=parent_button_size['width'],
                  fg_color=COLORS['success'], hover_color=COLORS['accent'], corner_radius=10,
                  state="normal")
    body_frame.top_panel.confirm_btn.configure(
        command=lambda: on_confirm(body_frame.top_panel.confirm_btn, frame, genotype_class)
    )
...
def confirm_inputfiles(frame, genotype_class, btn):
