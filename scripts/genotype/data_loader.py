...
def create_header(frame):
    ...
    label = ctk.CTkLabel(header_frame, text="■ Data Loading", font=header_font,
...
def create_body(parent, frame):
    ...
    bottom_panel.grid(row=1, column=0, sticky="nsew", padx=body_frame.padx,pady=(5,0))
    ...
    ctk.CTkLabel(top_panel, text="Read type: ", font = bfont, text_color="white").grid(row=row,column=0,padx=body_frame.padx,pady=(1,1),sticky="e")
    ctk.CTkRadioButton(top_panel, text="PE", font=bmfont, value="pe", variable=radio_readtype).grid(row=row, column=2, pady=(1,1), padx=(20,80), sticky="ew")  # Stick to right side of cell
    ctk.CTkRadioButton(top_panel, text="SE", font= bmfont, value="se", variable=radio_readtype).grid(row=row, column=2, pady=(1,1), padx=(80,0), sticky="ew")
    ...
    # Label
    ctk.CTkLabel(top_panel, text="Input folder:", font=bfont, text_color="white").grid(row=row, column=0, padx=body_frame.padx, pady=(1,1), sticky="e")
    ...
    # Browse Button
    input_browse_btn = ctk.CTkButton(top_panel, text="Browse", font=brfont, height=child_button_size['height'], width=child_button_size['width'],
                  corner_radius=8, fg_color=COLORS['primary'], hover_color=COLORS['secondary'])
    input_browse_btn.configure(command=lambda btn=input_browse_btn: indir_browser(top_panel.in_entry, "input", btn))
    ...
    ctk.CTkLabel(top_panel, text="Sample file:", font=bfont, text_color="white").grid(row=row, column=0, padx=body_frame.padx, pady=(1,1), sticky="e")
    ...
    sample_browse_btn = ctk.CTkButton(top_panel, text="Browse",font=brfont, height=child_button_size['height'], width=child_button_size['width'],
                  corner_radius=8, fg_color=COLORS['primary'], hover_color=COLORS['secondary'])
    sample_browse_btn.configure(command=lambda btn=sample_browse_btn: infile_browser(top_panel.sample_entry, "index", btn))
    sample_browse_btn.grid(row=row, column=1, pady=(1,1), sticky="w")
    ...
    ctk.CTkLabel(top_panel, text="Loci file:", font= bfont, text_color="white").grid(row=row, column=0, padx=body_frame.padx, pady=(1,1), sticky="e")
    ...
    loci_browse_btn = ctk.CTkButton(top_panel, text="Browse", font=brfont, height=child_button_size['height'], width=child_button_size['width'],
                  corner_radius=8, fg_color=COLORS['primary'], hover_color=COLORS['secondary'])
    ...
    ctk.CTkLabel(top_panel, text="Sex file (optional):", font=bfont, text_color="white").grid(row=row, column=0, padx=body_frame.padx, pady=(1,1), sticky="e")
    ...
    sex_browse_btn = ctk.CTkButton(top_panel, text="Browse",font=brfont, height=child_button_size['height'], width=child_button_size['width'],
                  corner_radius=8, fg_color=COLORS['primary'], hover_color=COLORS['secondary'])
    sex_browse_btn.configure(command=lambda btn=sex_browse_btn: infile_browser(top_panel.sex_entry, "index", btn))
    sex_browse_btn.grid(row=row, column=1, pady=(1,1), sticky="w")
    ...
    ctk.CTkCheckBox(top_panel, text="reverse complement", variable = sex_rev_var, font=brfont, text_color="white").grid(row=row, column=3, padx=(0, 150), sticky="w")
    ...
    ctk.CTkLabel(top_panel, text="training model (optional):", font=bfont, text_color="white").grid(row=row, column=0, padx=body_frame.padx, pady=(1,1), sticky="e")
    ...
    model_browse_btn = ctk.CTkButton(top_panel, text="Browse",font= brfont, height=child_button_size['height'], width=child_button_size['width'],
                  corner_radius=8, fg_color=COLORS['primary'], hover_color=COLORS['secondary'])
    model_browse_btn.configure(command=lambda btn=model_browse_btn: infile_browser(top_panel.model_entry, "index", btn))
    model_browse_btn.grid(row=row, column=1, pady=(1,1), sticky="w")
    ...
    ctk.CTkLabel(top_panel, text="Output folder:", font=bfont, text_color="white").grid(row=row, column=0, padx=body_frame.padx, pady=(1,1), sticky="e")
    ...
    output_browse_btn = ctk.CTkButton(top_panel, text="Browse",font=brfont, height=child_button_size['height'], width=child_button_size['width'],
                  corner_radius=8, fg_color=COLORS['primary'], hover_color=COLORS['secondary'])
    output_browse_btn.configure(command=lambda btn=output_browse_btn: outfile_browser(top_panel.out_entry, False, btn))
    ...
    output_var.trace_add("write", lambda *args: genotype_class.get_parameter().set_outputdir(output_var.get()))
      ...
      #confirm button
    ctk.CTkButton(top_panel, text="Confirm",font=confirm_button_font, height=parent_button_size['height'], width=parent_button_size['width'],
                  fg_color=COLORS['primary'], hover_color=COLORS['secondary'], corner_radius=10,
                  command = lambda :confirm_inputfiles(frame, tree_frame, genotype_class)).grid(row=row, column=3, pady=(5,5), padx=(20,10), sticky="w")
...
def confirm_inputfiles(frame, tree_frame, genotype_class):
    if genotype_class.get_parameter().get_inputdir() is None or not os.path.exists(genotype_class.get_parameter().get_inputdir()):
        modern_messagebox.showerror(frame, "Error", "Input directory does not exist!")
        return
    if genotype_class.get_parameter().get_samplefile() is None or not os.path.exists(genotype_class.get_parameter().get_samplefile()):
        modern_messagebox.showerror(frame, "Error", "Sample file does not exist!")
        return
    if genotype_class.get_parameter().get_locifile() is None or not os.path.exists(genotype_class.get_parameter().get_locifile()):
        modern_messagebox.showerror(frame, "Error", "Loci file does not exist!")
        return
    if genotype_class.get_parameter().get_outputdir() is None or not os.path.exists(genotype_class.get_parameter().get_outputdir()):
        modern_messagebox.showerror(frame, "Error", "Output directory does not exist!")
        return
    ...
    for col in tree_frame.file_tree['columns']:
        # Get the heading text width
        heading_text = tree_frame.file_tree.heading(col)['text']
        heading_width = len(str(heading_text)) * 11
        
        # Get max content width from all rows
        max_content_width = heading_width
        for item in tree_frame.file_tree.get_children():
            values = tree_frame.file_tree.item(item)['values']
            col_index = tree_frame.file_tree['columns'].index(col)
            if col_index < len(values):
                content_width = len(str(values[col_index])) * 9
                max_content_width = max(max_content_width, content_width)
        
        # Set column width with padding
        final_width = min(max(max_content_width + 20, heading_width + 20), 500)
        tree_frame.file_tree.column(col, anchor=tk.CENTER, width=final_width, minwidth=heading_width + 20, stretch=False)
