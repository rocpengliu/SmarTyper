...
def create_header(frame):
    ...
    label = ctk.CTkLabel(header_frame, text="Result summary", font=("Helvetica", 30, "bold"),
...
def create_body(parent, frame):
    ...
    ctk.CTkLabel(top_panel, text="For microhap identification", font=body_frame.bfont, text_color="white").grid(row=row, column=0, padx=body_frame.padx, pady=(1,1), sticky="e")
    ctk.CTkButton(top_panel, text="Let's Go",font=("Helvetica", 18, "bold"), height=40, width=120,).grid(row=row, column=1, pady=(1,1), padx=(0,10), sticky="w")
                 # command = lambda :go_button(parent)).grid(row=row, column=1, pady=(1,1), padx=(0,10), sticky="w")
    return body_frame
